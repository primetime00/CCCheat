...
ConnectionManager *ConnectionManager::connectionManager = 0;
...
{
	if (ConnectionManager::connectionManager == 0)
		return -1;
	if (!ConnectionManager::connectionManager->requestConnection(sock, ipAddress, hostVersion))
		return -1;
	connected = true;
	return 0;
}
...
int CCAPI::disconnect(void)
{
	...
	if (ConnectionManager::connectionManager == 0)
		return -1;
	ConnectionManager::connectionManager->releaseConnection(sock);
	return 0;
}
...
bool ConnectionManager::requestConnection(int &sock, string ipAddress, int hostVersion)
{
	struct sockaddr_in destination;
	for (int i=0; i<MAX_CONNECTIONS; ++i)
	{
		if (connection[i].available && connection[i].sock == 0) //we need to connect
		{
			...
			connection[i].sock = sock;
			connection[i].available = false;
			return true;
		}
		else if (connection[i].available) //we found it
		{
			sock = connection[i].sock;
			connection[i].available = false;
			return true;
		}
	}
...
void ConnectionManager::releaseConnection(int sock)
{
	for (int i=0; i<MAX_CONNECTIONS; ++i)
	{
		if (connection[i].sock == sock) // this is the one we are releasing
		{
			connection[i].available = true;
		}
	}
}
...
void ConnectionManager::killAllConnections()
{
	for (int i=0; i<MAX_CONNECTIONS; ++i)
	{
		if (connection[i].sock != 0) // disconnect
		{
