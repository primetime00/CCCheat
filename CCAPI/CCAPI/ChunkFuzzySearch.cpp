...
#define DO_COMPARE(_type, val1, val2, loc, base)\
	signage = TEST_SIGN_NULL;\
	if (CMP(_type, val1, val2, loc))\
		signage |= TEST_SIGN_YES;\
	if (CMP_UNSIGNED(_type, val1, val2, loc))\
...
void ChunkFuzzySearch::digest(char *newMemory, char *prevMemory, unsigned long length, unsigned long address)
{
	...
	if (valueByteLength == 1) //we are a character search, we good
	{
		for (i=0; i<length; i++)
		{
			DO_COMPARE(char, &newMemory[i], &prevMemory[i], address+i, address);
		}
	}
	else if (valueByteLength == 2)
	{
		...
		else
		{
			...
			short tmp1 = BSWAP16(*(short*)&newBytes[0]);
			short tmp2 = BSWAP16(*(short*)&prevBytes[0]);
			DO_COMPARE(short, &tmp1, &tmp2, address-1, prevAddress);
		}
		for (i=0; i<length-1; i++)
		{
			short tmp1 = BSWAP16(*(short*)&newMemory[i]);
			short tmp2 = BSWAP16(*(short*)&prevMemory[i]);
			DO_COMPARE(short, &tmp1, &tmp2, address+i, address);
		}
	...
	else if (valueByteLength == 4)
	{
		...
		else
		{
			...
			for (int j=0; j<3; j++)
			{
				long tmp1 = BSWAP32(*(unsigned long*)&newBytes[j]);
				long tmp2 = BSWAP32(*(unsigned long*)&prevBytes[j]);
				if (isFloat)
				{
					DO_COMPARE_FLOAT(&tmp1, &tmp2, address-3+j, prevAddress);
				}
				else
				{
					DO_COMPARE(long, &tmp1, &tmp2, address-3+j, prevAddress);
				}
			}
		}
		for (i=0; i<length-3; i++)
		{
			long tmp1 = BSWAP32(*(long*)&newMemory[i]);
			long tmp2 = BSWAP32(*(long*)&prevMemory[i]);
			if (isFloat)
			{
				DO_COMPARE_FLOAT(&tmp1, &tmp2, address+i, address);
			}
			else
			{
				DO_COMPARE(long, &tmp1, &tmp2, address+i, address);
			}
		}
